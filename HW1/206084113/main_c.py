...
for layer_name in layers:
    ...
    with torch.no_grad():
        ...
        for _ in range(consts.BF_PER_LAYER):
            ...
            flip_id = np.random.randint(0, W.numel())
            W = W.flatten()
            # flip
            flip_bit = W[flip_id]
            new_bit, bit_id = utils.random_bit_flip(flip_bit)
...
plt.figure()
plt.boxplot([RADs_bf_idx[bit_id] for bit_id in range(32)])
