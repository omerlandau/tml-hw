...
def free_adv_train(model, data_tr, criterion, optimizer, lr_scheduler, \
    ...
    delta = torch.zeros_like(tmp_input).to(device)
    ...
    for epoch in range(totatl_updates_num):
        ...
        if epoch%scheduler_step_iters == 0:
            lr_scheduler.step()
...
class SmoothedModel():
    ...
    def certify(self, x, n0, n, alpha, batch_size):
        ...
        l_b = proportion_confint(p_c, n, alpha=2*alpha, method="beta")
...
class NeuralCleanse:
    ...
    def find_candidate_backdoor(self, c_t, data_loader, device):
        ...
        mask = torch.rand((1,1,self.dim[2], self.dim[3]), requires_grad=True).to(device)
        trigger = torch.rand(self.dim).to(device)

        # run self.niters of SGD to find (potential) trigger and mask

        optimizer = torch.optim.SGD(params=self.model.parameters(),lr=self.step_size)
        ...
        while sgd_count <= self.niters:
            for _, data in enumerate(data_loader, 0):
                ...
                batch_trigger = (mask*trigger)
                batch_mask = (torch.ones((1,1,self.dim[2], self.dim[3]), requires_grad=True).to(device) - mask)
                t_i = batch_mask*inputs + batch_trigger
                ...
                print(labels.size())
                m_labels = torch.tensor(labels.size())
                m_labels[c_t] = 1
                m_labels.to(device)
                ...
                loss += self.lambda_c*torch.norm(mask, p=1).to(device)
                loss.backward()
